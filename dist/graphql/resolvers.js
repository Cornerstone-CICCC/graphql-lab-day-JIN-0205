...
exports.resolvers = {
    ...
    Product: {
        customers: (parent) => __awaiter(void 0, void 0, void 0, function* () {
            const getAllOrders = yield order_controller_1.default.getOrders();
            const filteredOrders = getAllOrders.filter((order) => order.productId.toString() === parent.id);
            const customerIds = filteredOrders.map((order) => order.customerId);
            const customers = customerIds.map((id) => customer_controller_1.default.getCustomerById(id));
            return customers;
        }),
    },
    Customer: {
        products: (parent) => __awaiter(void 0, void 0, void 0, function* () {
            const getAllOrders = yield order_controller_1.default.getOrders();
            const filteredOrders = getAllOrders.filter((order) => order.customerId.toString() === parent.id);
            const productIds = filteredOrders.map((order) => order.productId);
            const products = productIds.map((id) => product_controller_1.default.getProductById(id));
            return products;
        }),
    },
    Order: {
